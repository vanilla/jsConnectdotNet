 ...
 */
     ...
     */
        public string GenerateResponseLocation(string requestJwt) {
            ...
            var state = (IDictionary<string, object>) (request.Payload.ContainsKey(FIELD_STATE)
                ? request.Payload[FIELD_STATE]
     ...
     */
        protected JwtSecurityToken JwtDecode(string jwt) {
            ...
            var key = new SymmetricSecurityKey(System.Text.Encoding.UTF8.GetBytes(this._signingSecret));
            key.KeyId = this.GetSigningClientID();
            // key.CryptoProviderFactory.CustomCryptoProvider = new CryptoProvider();
            
            var supported = key.CryptoProviderFactory.IsSupportedAlgorithm(ALG_HS256, key);
            var supported2 = key.CryptoProviderFactory.IsSupportedAlgorithm(SecurityAlgorithms.Sha256);
        ...
        private IEnumerable<SecurityKey> ResolveSigningKey(string token, SecurityToken securitytoken, string kid,
     ...
     */
        public JsConnectV3 SetSigningCredentials(string clientId, string secret) {
            ...
            if (Encoding.UTF8.GetBytes(secret).Length * 8 <
                SymmetricSignatureProvider.DefaultMinimumSymmetricKeySizeInBits) {
     ...
     */
        ...
        private class CryptoProvider : ICryptoProvider {
            ...
            private static string TranslateAlgorithm(string algorithm) {
                return algorithm switch {
                    ALG_HS256 => SecurityAlgorithms.Sha256,
                    ALG_HS512 => SecurityAlgorithms.Sha384,
                    ALG_HS384 => SecurityAlgorithms.Sha512,
                    _ => algorithm
                };
            }
            ...
            public bool IsSupportedAlgorithm(string algorithm, params object[] args) {
                ...
                return _factory.IsSupportedAlgorithm(algorithm);
            }
            ...
            public object Create(string algorithm, params object[] args) {
                ...
                object r;
                if (args.Length == 0) {
                    r = _factory.CreateHashAlgorithm(algorithm);
                } else if (args.Length > 0 && args[0] is SecurityKey key) {
                    r = _factory.CreateForVerifying(key, algorithm);
                } else {
                  throw new ArgumentException("Don't understand how to create a signature provider.");  
                }
            ...
            public void Release(object cryptoInstance) {
                if (cryptoInstance is HashAlgorithm hash) {
                    _factory.ReleaseHashAlgorithm(hash);
                } else if (cryptoInstance is SignatureProvider signatureProvider) {
                    _factory.ReleaseSignatureProvider(signatureProvider);
                } else {
                    throw new ArgumentException("Don't know how to release: " + cryptoInstance.ToString());
                }
            }
        }
    }
